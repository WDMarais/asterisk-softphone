...
describe("parse_sip_headers (§2.2)", () => {
    ...
    it("accumulates both warnings when From and X-Queue are absent", () => {
        const result = parseSipHeaders(buildInvite({ ...FULL_HEADERS, From: undefined, "X-Queue": undefined }));
        expect(result.ok).toBe(true);
        if (!result.ok)
            return;
        expect(result.warnings).toEqual(expect.arrayContaining(["From header missing", "X-Queue missing"]));
        expect(result.value.caller_id).toBeNull();
        expect(result.value.queue_name).toBeNull();
    });
    ...
    it("matches header names case-insensitively (RFC 3261 §7.3.1)", () => {
        const result = parseSipHeaders(buildInvite({
            ...FULL_HEADERS,
            From: undefined,
            "X-Queue": undefined,
